...
// Nosy Neighbor Globals
static int nosy_percent = 0;
static int nosy_time_ms = 0;
...
void run_benchmark() {
  ...
  for (auto & [name, pattern]: patterns) {
    for (int sample_iter = 0; sample_iter < nsamples; sample_iter++) {
      ...
      double apply = -1.0;
      double nosy_count = 0.0; // Tracks nose iterations for this sample
