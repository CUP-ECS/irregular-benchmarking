...
enum distribution { GAUSSIAN, EMPIRICAL, STATIC_VALUE };
typedef enum distribution distribution_t;

enum halo { IMPORT, EXPORT };

enum comm { MPIADVANCE, MPIS };
...
struct Pattern {
  ...
  std::map<int, std::map<int, double>>
      dist_to_neighbors;           // each inner map normalized
  std::map<int, double> neighbors; // normalized
  ...
  if (!HiFi) {
    {
      ...
      nneighbors_max =
          std::max(p.comm_partners.rbegin()->first, nneighbors_max);
    }
...

    {
      ...
      std::map<int, int> temp;
      for (auto &[k, v] : j.at("neighbors").items()) {
        temp[std::stoi(k)] = v.get<int>();
      }
      ...
      for (auto &[k, v] : temp)
        total += v;
      for (auto &[k, v] : temp) {
        p.neighbors[k] = (total > 0) ? (double)v / total : 0.0;
      }
    }
...
void run_benchmark() {
  ...
  for (auto &[name, pattern] : patterns) {
    ...
    for (int sample_iter = 0; sample_iter < nsamples; sample_iter++) {
      ...
      int nneighborsV = -1;
      if (HiFi) {
        nneighborsV = pattern.local_comm_size
      } else {
      ...
      for (int i = 0; i < nneighborsV; ++i) {
        ...
        while (true) {
          ...
          if (Hifi) {
            ...
            node = sample_from_map(pattern.neighbors);

          } else {
            int distanceToN =
                sample_from_map(pattern.dist_to_neighbors[nneighborsV]);
      ...
      while (it_data != neighbors_data.end() &&
      ...
      if (comm_type == MPIADVANCE) {
        if (halo_type == EXPORT) {
          ...
          Cabana::Halo<MemorySpace, Cabana::Export, Cabana::LocalityAware> halo(
              MPI_COMM_WORLD, num_tuple, export_ids, export_ranks);
          ...
          TIME_START = std::chrono::high_resolution_clock::now();
          Cabana::Halo<MemorySpace, Cabana::Import, Cabana::LocalityAware> halo(
              MPI_COMM_WORLD, num_tuple, export_ids, export_ranks);
          TIME_END = std::chrono::high_resolution_clock::now();
          duration = TIME_END - TIME_START;
          haloTime = duration.count();
        ...
        if (halo_type == EXPORT) {
          ...
          TIME_START = std::chrono::high_resolution_clock::now();
          Cabana::Halo<MemorySpace, Cabana::Export, Cabana::Mpi> halo(
              MPI_COMM_WORLD, num_tuple, export_ids, export_ranks);
          TIME_END = std::chrono::high_resolution_clock::now();
          duration = TIME_END - TIME_START;
          haloTime = duration.count();
          ...
          TIME_START = std::chrono::high_resolution_clock::now();
          Cabana::Halo<MemorySpace, Cabana::Import, Cabana::Mpi> halo(
              MPI_COMM_WORLD, num_tuple, export_ids, export_ranks);
          TIME_END = std::chrono::high_resolution_clock::now();
          duration = TIME_END - TIME_START;
          haloTime = duration.count();
      ...
      std::chrono::duration<double, std::milli> halo_gather_time =
          TIME_END_Halo - TIME_START_HALO;
      double halo_gather = halo_gather_time.count();

#define DATA_SIZE 7
      double local_vals[DATA_SIZE] = {
          haloTime,    resizeTime,          gatherTime,  apply,
          halo_gather, (double)nneighborsV, (double)inum};
      ...
      // Perform reductions
      MPI_Reduce(local_vals, min_vals, DATA_SIZE, MPI_DOUBLE, MPI_MIN, 0,
                 MPI_COMM_WORLD);
      MPI_Reduce(local_vals, max_vals, DATA_SIZE, MPI_DOUBLE, MPI_MAX, 0,
                 MPI_COMM_WORLD);
      MPI_Reduce(local_vals, sum_vals, DATA_SIZE, MPI_DOUBLE, MPI_SUM, 0,
                 MPI_COMM_WORLD);
      ...
      if (comm_rank == 0) {
        ...
        const char *labels[DATA_SIZE] = {
            "haloTime",         "resizeTime", "gatherTime", "apply",
            "halo_gather_time", "nneighbors", "data_sent"};
...
// Function to set a value based on a command-line argument and check its
// validity.
void setAndCheckValue(int &value, TCLAP::ValueArg<int> &arg,
                      const char *errorMessage, int minValue = 0,
                      int maxValue = INT_MAX) {
  int tempValue = arg.getValue();
  // If the value from the argument is not -1 (indicating it was set by the
  // user), update the 'value' reference with the new value.
  if (tempValue != -1) {
    value = tempValue;
  }

  // Check if the value is within the allowed range (between minValue and
  // maxValue).
...
void parseArgs(int argc, char **argv) {
  ...
  try {
    ...
    TCLAP::ValueArg<std::string> filepathArg(
        "f", "filepath", "Path to the BENCHMARK_CONFIG file", true, "NOFILE",
        "string");
    TCLAP::ValueArg<int> samplesArg("I", "samples",
                                    "Number of random samples to generate",
                                    false, 25, "int");
    TCLAP::ValueArg<int> iterationsArg("i", "iterations",
                                       "Number of updates each sample performs",
                                       false, niterations, "int");
    TCLAP::ValueArg<int> seedArg(
        "S", "seed",
        "Positive integer to be used as seed for random number generation",
        false, -1, "int");
    TCLAP::SwitchArg useedArg("q", "unique-seed", "unique seed per rank", true);
    TCLAP::SwitchArg reportParamsArg(
        "r", "report-params",
        "Enables parameter reporting for use with analysis scripts", false);
    TCLAP::ValueArg<std::string> distributionArg(
        "d", "distribution",
        "Choose from: gaussian (default), empirical or static", false,
        "gaussian", "string");
    TCLAP::ValueArg<std::string> commArg(
        "c", "comm", "Choose from: MPIA|A|a (default) or MPI|M|m", false,
        "MPIA", "string");
    TCLAP::ValueArg<std::string> INorOUTArg(
        "x", "type", "Choose from: EXPORT|E|e (default) or IMPORT|I|i", false,
        "EXPORT", "string");
    TCLAP::ValueArg<std::string> ALLTOALLV(
        "a", "alltoallv", "Choose from: STANDARD|S|s (default) or LOCALITY|L|l",
        false, "STANDARD", "string");
    TCLAP::ValueArg<std::string> CRS(
        "C", "crs",
        "Choose CRS method: default| nonblocking | personalized | "
        "personalized_loc | nonblocking_loc |rma",
        false, "default", "string");
    TCLAP::SwitchArg hifi("F", "hifi", "increases fedity of patterns", false);
    cmd.add(hifi);
    ...
    if (filepath != "NOFILE") {
      ...
      if (Hifi) {
        ...
        if (pos != std::string::npos) {
          filepath.replace(pos, 1, std::to_string(rank));
        } else {
          std::cerr
              << "Error: File path does not contain '*' to replace! (Rank "
              << rank << ")" << std::endl;
          // Optionally, you can exit here if it's fatal
    ...
    setAndCheckValue(niterations, iterationsArg,
    ...
    if (discovery == "rma") {
      ...
      exitError("ERROR: Invalid pattern choice [default,nonblocking, "
                "personalized, personalized_loc, nonblocking_loc]\n");
    }
