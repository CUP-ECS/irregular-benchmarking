...
static bool unique_per_rank_message = 0;
static bool senderDriven = true;

struct Pattern {
    std::map<int, std::map<int, std::map<int, int>>> solo; // type of send to neibor to d
...
void from_json(const json & j, Pattern & p) {
    ...
    if(unique_per_rank_message){
        int key1 = std::stoi(k1);

        for (auto& [k2, v2] : v1.items()) {
            int key2 = std::stoi(k2);

            for (auto& [k3, v3] : v2.items()) {
                int key3 = static_cast<int>(std::stod(k3)); // "18432.00" -> 18432
                int value = v3.get<int>();

                p.solo[key1][key2][key3] = value;
            }
        }
...
void run_benchmark_single_file() {
...
void run_benchmark_single_mutiple_file() {
    if(senderDriven){
     auto TIME_START = std::chrono::high_resolution_clock::now();
    auto TIME_END = std::chrono::high_resolution_clock::now();
    ...
    for (auto & [name, pattern]: patterns) {
        for (int sample_iter = 0; sample_iter < nsamples; sample_iter++) {
            ...
            for (int sendtype : {16}) {
                auto it = pattern.solo.find(sendtype);
                if (it != pattern.solo.end()) {
                    for (const auto& [neighbor, dataTOamount] : it->second) {
                        for (const auto& [data, amount] : dataTOamount) {

                            for (int i = 0; i <= amount; i++) {
                               nneighborsV++;
                               neighbors_data.push_back(data);
        ...
        if (filepath != "NOFILE") {
            ...
            try {
                ...
                if (path.find('*') != std::string::npos) {
                    ...
                    // make a copy
                    std::string rank_path = filepath;


                    rank_path.replace(pos, 1, std::to_string(rank));
                    std::filesystem::path p(rank_path);
